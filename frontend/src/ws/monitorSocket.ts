...
export type MonitorSocketStatus = {
...
type StatusListener = (status: MonitorSocketStatus) => void;

class MonitorSocketSingleton {
  private ws: WebSocket | null = null;
  private session: SessionInfo | null = null;
  private sessionKey = "";
  private status: MonitorSocketStatus = { kind: "disconnected" };
  ...
  private emitStatus(next: MonitorSocketStatus) {
    this.status = next;
  ...
  private connect() {
    ...
    const ws = new WebSocket(wsUrl(this.session, "/ws/monitor"));
    this.ws = ws;
...
const monitorSocketSingleton = new MonitorSocketSingleton();

export function getMonitorSocket() {
  return monitorSocketSingleton;
}
