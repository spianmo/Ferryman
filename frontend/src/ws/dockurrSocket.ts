...
export type DockurrSocketStatus = {
...
type StatusListener = (status: DockurrSocketStatus) => void;

class DockurrSocketSingleton {
  private ws: WebSocket | null = null;
  private session: SessionInfo | null = null;
  private sessionKey = "";
  private status: DockurrSocketStatus = { kind: "disconnected" };
  ...
  private emitStatus(next: DockurrSocketStatus) {
    this.status = next;
  ...
  private connect() {
    ...
    const ws = new WebSocket(wsUrl(this.session, "/ws/dockurr"));
    this.ws = ws;
    ...
    ws.onopen = () => {
      ...
      this.send({ action: "list", request_id: `snapshot-${Date.now()}` });
    };
...
const dockurrSocketSingleton = new DockurrSocketSingleton();

export function getDockurrSocket() {
  return dockurrSocketSingleton;
}
