...
export type TunnelSocketStatus = {
...
type StatusListener = (status: TunnelSocketStatus) => void;

class TunnelSocketSingleton {
  private ws: WebSocket | null = null;
  private session: SessionInfo | null = null;
  private sessionKey = "";
  private status: TunnelSocketStatus = { kind: "disconnected" };
  ...
  private emitStatus(next: TunnelSocketStatus) {
    this.status = next;
  ...
  private connect() {
    ...
    const ws = new WebSocket(wsUrl(this.session, "/ws/tunnel"));
    this.ws = ws;
...
const tunnelSocketSingleton = new TunnelSocketSingleton();

export function getTunnelSocket() {
  return tunnelSocketSingleton;
}
