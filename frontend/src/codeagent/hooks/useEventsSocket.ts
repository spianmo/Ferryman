...
export type EventsSocketSubscription = {
    all?: boolean
    sessionId?: string
    machineId?: string
}
...
type SessionPatch = Partial<Pick<Session, 'active' | 'thinking' | 'activeAt' | 'updatedAt' | 'permissionMode' | 'modelMode' | 'reasoningEffort'>>
...
function getSessionPatch(value: unknown): SessionPatch | null {
    ...
    if (typeof value.reasoningEffort === 'string') {
        patch.reasoningEffort = value.reasoningEffort as Session['reasoningEffort']
        hasKnownPatch = true
    }
...
function hasUnknownSessionPatchKeys(value: unknown): boolean {
    ...
    const knownKeys = new Set(['active', 'thinking', 'activeAt', 'updatedAt', 'permissionMode', 'modelMode', 'reasoningEffort'])
...
function buildEventsWsUrl(
    baseUrl: string,
    subscription: EventsSocketSubscription,
    ...
    const path = `/ws/codeagent/events?${params.toString()}`
    try {
...
export type UseEventsSocketOptions = {
    enabled: boolean
    baseUrl: string
    token?: string
    subscription?: EventsSocketSubscription
...
export function useEventsSocket(options: UseEventsSocketOptions): { subscriptionId: string | null } {
    ...
    useEffect(() => {
        ...
        setSubscriptionId(null)
        const url = buildEventsWsUrl(options.baseUrl, {
            ...subscription,
            sessionId: subscription.sessionId ?? undefined
        }, getVisibilityState(), options.token)
