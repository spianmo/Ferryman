...
import { getCodeAgentRootElement } from '@/lib/dom'
...
export function SessionActionMenu(props: SessionActionMenuProps) {
    ...
    const updatePosition = useCallback(() => {
        ...
        const rootEl = getCodeAgentRootElement()
        const rootRect = rootEl?.getBoundingClientRect() ?? null
        const viewportWidth = rootRect ? rootRect.width : window.innerWidth
        const viewportHeight = rootRect ? rootRect.height : window.innerHeight
        const padding = 8
        const gap = 8

        const anchorX = rootRect ? anchorPoint.x - rootRect.left : anchorPoint.x
        const anchorY = rootRect ? anchorPoint.y - rootRect.top : anchorPoint.y
        ...
        let top = openAbove ? anchorY - menuRect.height - gap : anchorY + gap
