...
function getContextWarning(
    ...
    const usedTokens = Math.max(0, Math.round(contextSize))
    const limitTokens = Math.max(1, Math.round(maxContextSize))
    const remainingTokens = Math.max(0, limitTokens - usedTokens)
    const percentageRemaining = Math.min(100, Math.max(0, (remainingTokens / limitTokens) * 100))
...
export function StatusBar(props: {
    ...
    const contextWarning = useMemo(
        () => {
            ...
            const configuredBudget = getContextBudgetTokens({
